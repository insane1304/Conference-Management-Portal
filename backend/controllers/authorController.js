...
const { authorRegisterValid, authorLoginValid ,createConfValid, editProfileValid, updatePassValid,emailValid, resetPassValid, updateConfValid } = require("./validation");

const {doUpload} = require("./uploadFile")
...
const authorRegister = async (req, res) => {
    try {
        ...
        // validation
        const { error } = authorRegisterValid(req.body);
        // console.log("hi");
        if (error) {
        ...
        const emailExist = await Author.findOne({ email: givenEmail });
...
const authorLogin = async (req, res) => {
    ...
    // validation
    const { error } = authorLoginValid(req.body);
...
const editProfile = async (req, res) => {
  try {
    ...
    const { error } = editProfileValid(req.body);
    // console.log("hi");
    if (error) {
...
const updatePassword = async (req, res) => {
    try {
        ...
        const { error } = updatePassValid(req.body);
        if (error) {
        ...
        await Author.updateOne({_id:id},{password:newpassword});
        res.status(201).json(author);
    ...
    try {
        ...
        if (f_url.length===24){
          ...
          if( f_url.match(/^[0-9a-fA-F]{24}$/)){
            const profile= await Author.findOne({ _id: f_url });
            if (!profile){
              result.push([]);
            }
            else{
              result.push(profile);
              const conferences= await Paper.find({author_id:profile._id}).sort({creation_date:-1});
...
const myConferences = async (req, res) => {
    try {
        ...
        const conferences= await Conference.find({$or:[{chairperson: id},{authors:id},{reviewers:id}]});
...
const createConf = async (req, res) => {
    try {
        ...
        const { error } = createConfValid(req.body);
        // console.log("hi");
        if (error) {
          return res.status(400).json(error.details[0].message);
        }
...
try{
           ...
           console.log(token);
           const author = await Author.findOne({forgetPass:token});
           if(!author){
             res.status(400).send("Invalid Link");
           }
           // console.log(`${buf.length} bytes of random data: ${buf.toString('hex')}`);

           const { error } = resetPassValid(req.body);
           if (error) {
             if(error.details[0].message=="\"Confirm Password\" must be [ref:newPass]"){
               return res.status(400).json("Confirm Password must be same as  New Password!")
             }
...
const updateConf = async (req, res) => {
    try {
        ...
        const { error } = updateConfValid(req.body);
        if (error) {
          return res.status(400).json(error.details[0].message);
        }
...
const chechAddAuthor= (authors,id)=>{
  let check=false;
  return Promise.all(
    authors.map(async item => {
...
const addAuthor = async (req, res) => {
    try {
        ...
        const { error } = emailValid(req.body);
        if (error) {
          return res.status(400).json(error.details[0].message);
        }
        ...
        const newAuthor= await Author.findOne({email:email});
        if(!newAuthor){
          res.status(400).json("No such user exist");
        }
        console.log(newAuthor);
        var check = false;
        check=check|(await chechAddAuthor(conference.authors,newAuthor._id));
        var check2=false;
        check2=check2| (await chechAddAuthor(conference.reviewers,newAuthor._id));
        console.log(check);
        console.log(check2);
        if(check || check2){
...
const addReviewer = async (req, res) => {
    try {
        ...
        const newReviewer= await Author.findOne({email:email});
        if(!newReviewer){
          res.status(400).json("No such user exist");
        }
        var check=false
        check=check|(await chechAddAuthor(conference.authors,newReviewer._id));
        var check2=false;
        check2=check2| (await chechAddAuthor(conference.reviewers,newReviewer._id));
        console.log(check);
        console.log(check2);
        if(check || check2){
          res.status(400).json("User already exists in authors or reviewers")
...
try{
           ...
           const too=await Author.findOne({_id:paper.author_id})
           ...
           const sendToo = sendTo.replaceAll('\'', '');
             ...
             // subject: 'Feedback regarding your Submission'+ {paper.title},
             // const by= <a href=\"/profile/"+author._id+">"++"</a>";
             const q1="How well does the proposal meet the conderence theme ? : "+respObj[0].toString();
             const q2="What is overall conceptual quality of the paper ? : "+respObj[1].toString();
             const q3="Originality, novelity and innovation of the work ? : "+respObj[2].toString();
             const q4="Overall evaluation : "+respObj[3].toString();
             const q5="Grammar : "+respObj[4].toString();
             ...
             console.log(respObj);
              await transporter
                   .sendMail({
                     ...
                     html: `
                     <p>Feedback submitted by: <a href="http://localhost:3000/profile/${(author._id)}">${(author.firstName).toString()+" "+author.lastName.toString()}</a></p>
                     <p>${q1}</p>
                     <p>${q2}</p>
                     <p>${q3}</p>
                     <p>${q4}</p>
